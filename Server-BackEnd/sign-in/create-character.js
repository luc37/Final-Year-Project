...
const create = function(socket){
    socket.on('create character', function(data){
        let dup = false;
        database.connection.query('select * from user', function(err,users){
            if(err){
                console.log(err);
            } else{
                users.forEach(user => {
                    if(user.characterName === data.characterName){
                        socket.emit('already a character with this name', null);
                        dup = true;
                    }
                });

                if(!dup){
                    let query = 'INSERT into user (characterName, password, email, health, bullets, gunPower, aim, lookDirection, maxHealth, smell, clumsiness, roomId)';
                    let q2 = 'Values (\'' + data.characterName +'\' ,\''+ data.password +'\',\''+ data.email + '\', 10, 8, 2, 2, \'north\', 10, 0, 0, 1)';
